...
# Case 1: Setpoint change -> setpoint_change=True
# Case 2: Grid voltage change (mag and/or phase) -> setpoint_change=False
def pq_droop_disturbance(new_setpoints, x0, R, L, Vg, 
    ...
    # new_setpoints is a list with [new_ref_p, new_ref_q] or the new [Vg_d, Vg_q] after a disturbance 
    ...
    grid_phase_offset = np.angle(Vg[0] + Vg[1]*1j)
    ...
    grid_phase_of_t = np.ones(len(ts)) * grid_phase_offset

    # Calculate initial values
    Vdq0 = Vg + Zeq@x0 # Vdq of inverter in grid dq frame at t0
    v_nom = np.linalg.norm(Vdq0)
    delta0 = np.angle(Vdq0[0] + Vdq0[1]*1j) - grid_phase_offset
    ...
    for t in ts[1:]:
        ...
        Idq_plus = Yeq @ np.array([Vmag_plus - Vg_mag * np.cos(delta_plus - grid_phase_offset),
                                   Vg_mag * np.sin(delta_plus - grid_phase_offset)])
        ...
        if i_norm > Imag_lim:
            ...
            V_actual = np.array([Vg_mag * np.cos(delta_plus - grid_phase_offset),
                                 -Vg_mag * np.sin(delta_plus - grid_phase_offset)]) + Zeq @ Idq_plus # recalculate Vdq
        ...
        grid_phase_of_t[i] = grid_phase_offset
    ...
    for i,t in enumerate(Idq_of_t[0,:]):
        Idq_of_t[:,i] = rotate_device_frame_to_grid_frame(Idq_of_t[:,i],delta_of_t[i] - grid_phase_of_t[i])
        Vdqref_device = np.array([Vmag_of_t[i],0])
        Vdqref_of_t[:,i] = rotate_device_frame_to_grid_frame(Vdqref_device,delta_of_t[i] - grid_phase_of_t[i])
    Vdqactual_of_t = Vg.reshape(2,1) + Zeq @ Idq_of_t # NOTE: THIS IS INACCURATE WHEN Vg CHANGES MID-SIM
